  ...
  grid.innerHTML = items.map((s) => {
    ...
    return `
      <div class="stat-card" style="--tint:${t.bg}; --tint-ink:${t.ink}">
        <div class="stat-card__icon">${ICONS[s.icon]}</div>
        <div class="stat-card__value">${s.value}</div>
        <div class="stat-card__label">${s.label}</div>
        <span class="stat-card__delta stat-card__delta--${s.tone === "sun" ? "warn" : "up"}">${s.delta}</span>
      </div>`;
  }).join("");
}
...
function renderActivity() {
  document.getElementById("activityFeed").innerHTML = EDU_DATA.activity.map((a) => `
    <li>
      ...
      <span class="activity-feed__time">${a.time}</span>
    </li>`).join("");
}
...
function renderUpNext() {
  document.getElementById("upNextList").innerHTML = EDU_DATA.upNext.map((u) => `
    <li>
      <span class="upnext-list__date">${u.date}</span>
      <div>
...
function populateLibraryFilters() {
  ...
  EDU_DATA.subjects.forEach((s) => subjectSel.insertAdjacentHTML("beforeend", `<option value="${s}">${s}</option>`));
  EDU_DATA.grades.forEach((g) => gradeSel.insertAdjacentHTML("beforeend", `<option value="${g}">${g}</option>`));
    ...
    return `
      <article class="book-card">
        <div class="book-card__cover" style="background:${t.solid}">
          ${ICONS.pdf}
          <span class="book-card__type">${b.type}</span>
        </div>
        <div class="book-card__body">
          <span class="book-card__subject">${b.subject} · ${b.grade}</span>
          <h3 class="book-card__title">${b.title}</h3>
          <span class="book-card__meta">${b.size}</span>
        </div>
...
function renderPlaylistChips() {
  ...
  row.innerHTML = EDU_DATA.playlists.map((p) =>
    `<button class="chip ${p === state.activePlaylist ? "is-active" : ""}" data-playlist="${p}" type="button">${p}</button>`
  ).join("");
}
    ...
    return `
      <article class="video-card">
        ...
        <div class="video-card__body">
          ...
          <p class="video-card__teacher">${v.teacher} · ${v.playlist}</p>
          ${state.role === "student" ? `
...
function renderKanban() {
  ...
  Object.entries(cols).forEach(([status, tasks]) => {
    ...
    el.innerHTML = tasks.map((a) => `
      <div class="task-card" data-task="${a.id}">
        <span class="task-card__subject">${a.subject}</span>
        <div class="task-card__title">${a.title}</div>
        ${a.status === "done"
          ? `<span class="task-card__grade">${a.grade}/100</span>`
          : `<div class="task-card__due">Хугацаа: ${a.due}</div>
             <button class="btn btn--ghost btn--sm task-card__btn" data-toggle-submit="${a.id}">Даалгавар илгээх</button>
             <div class="submit-panel" id="submit-${a.id}">
               ...
               <button class="btn btn--primary btn--sm" style="margin-top:8px; width:100%" data-submit-task="${a.id}">Илгээх</button>
             </div>`
        }
      </div>`).join("");
  });
...
function renderSubmissions() {
  ...
  list.innerHTML = state.submissions.map((s) => `
    <button class="submission-row ${s.id === state.selectedSubmissionId ? "is-selected" : ""}" data-submission="${s.id}" type="button">
      <span class="avatar avatar--sm">${s.initials}</span>
      <span class="submission-row__body">
        <strong>${s.student} · ${s.title}</strong>
        <p>${s.subject} · илгээсэн ${s.submitted}</p>
      </span>
      <span class="status-pill status-pill--${s.status === "graded" ? "graded" : "pending"}">
        ${s.status === "graded" ? s.grade + "/100" : "Хүлээгдэж буй"}
      </span>
    </button>`).join("");
...
function applyRole() {
  ...
  // Toggle every element carrying a data-role attribute (buttons, panels…)
  document.querySelectorAll("[data-role]").forEach((el) => {
    if (el.id === "studentAssignments" || el.id === "teacherAssignments") return; // handled below
    const shouldShow = el.dataset.role === state.role;
    el.hidden = !shouldShow;
  });

  document.getElementById("studentAssignments").hidden = isTeacher;
  document.getElementById("teacherAssignments").hidden = !isTeacher;
  document.getElementById("asgSub").textContent = isTeacher
...
function bindEvents() {
  ...
  document.querySelectorAll(".role-switch__btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      applyRole();
      if (state.activeView === "assignments") { renderKanban(); renderSubmissions(); }
  ...
  // Kanban: open submission panel + submit
  document.getElementById("col-todo").parentElement.parentElement.addEventListener("click", (e) => {
    const toggleBtn = e.target.closest("[data-toggle-submit]");
    if (toggleBtn) {
      document.getElementById(`submit-${toggleBtn.dataset.toggleSubmit}`).classList.toggle("is-open");
    }
